...
namespace PvZ_Fusion_Translator.AssetStore
{
    public static class WarningStore
    {
        ...
        public static void WarningLoad()
        {
            ...
            var prefab = Resources.Load<GameObject>("UI/Prefabs/NoticeMenu");
            if (prefab == null)
            {
                Debug.LogError("Không tìm thấy prefab 'UI/Prefabs/NoticePauseMenu'");
                return;
            }

            if (GameAPP.canvas == null)
            {
                Debug.LogError("Debug: GameAPP.canvas not found!");
                return;
            }
            GameObject gameObject = UnityEngine.Object.Instantiate(prefab, GameAPP.canvas.transform);
